...
### Fixed
- **Сборка в CI**: полный `when` по перечислению скриптов в `AutoTtsDetector`
  (добавлены ветки `LATIN`/`OTHER`); ранее падала компиляция.
- **Карточки моделей MusicGen/NSynth** (`ModelsScreen`): раньше в состоянии
  RuntimeInDevelopment `DownloadableModelCard` вообще не рисовал ни одной
  кнопки (нет ни «Скачать», ни «Выбрать»). Теперь у таких карточек есть
  неактивная кнопка «Модель в разработке» + пояснение, почему недоступно.
  NSynth теперь показан во вкладке «Звуки».
- **Подсказки «как тестировать»** во вкладках «Музыка» и «Звуки»: локальный
  процедурный синтез работает без скачивания — достаточно вставить чип
  `<music>...</music>` / `<sfx>...</sfx>` в редакторе и запустить генерацию.
- **Тесты, попавшие в CI впервые**: русские часы `9:05` теперь читаются
  «девять ноль пять»; невалидные времена (99:99, 24:00) экранируются
  плейсхолдерами и остаются дословно, а не нормализуются числовым правилом;
  reverb-задержка больше не обрезается на 800 мс (короткие/длинные настроения
  дают разный материал); стекло диспетчеризуется также по «jar»/«shatter».

### Added
- **Auto-TTS: распознавание языка текста** (`com.t2v.tts.auto`). `AutoTtsDetector`
  определяет BCP-47 языка образца: скрипты решают по алфавиту
  (кириллица/гричи/арабский-персидский/иероглифы/кана/хангыль/деванагари/бенгальский),
  латинский текст ранжируется по частотности служебных слов (en/de/fr/es/it/pt/
  nl/cs/da/fi/hu/ca/tr). `AutoVoicePicker` выбирает голос по точному языку или
  общему первичного сабтегу (`en-GB` подходит для `en-US`). Покрыто JVM-тестами.
  → Следующий шаг (на устройстве): кнопка «Авто» в выборе языка/голоса.
- **Расширенные правила нормализации** (`TextNormalizer` / `Num2Words`):
  - время `H:MM` читается вслух (локализовано: ru «двадцать один сорок»,
    en «nine oh five» / «twelve o'clock»; невалидные 24:00/99:99 пропускаются);
  - правильные русские порядковые для форм `N-й/-я/-е/-ю/-го/-му/-м/-х`
    (5-й → пятый, 3-я → третья, 5-е → пятое, 1-ю → первую, 90-х → девяностых,
    21-й → двадцать первый); ранний порт `ordinalRu()` теперь даёт настоящие
    слова, а не кардинальное число.
